...
def provedor_ativo() -> tuple[str, str, bool, str]:
    """Retorna (provedor, modelo_padrao, disponivel, info). Groq tem prioridade
    quando ha chave; senao cai no Ollama local."""
    if _groq_key():
...
def _chat(system, prompt, modelo=None, json_mode=False, temperatura=0.2) -> str:
    prov, mod_def, _ok, _info = provedor_ativo()
    # se o modelo passado for de outro provedor (ex.: 'qwen2.5:3b' no Groq), usa o padrao
    if prov == 'groq' and (not modelo or ':' in str(modelo)):
        modelo = MODELO_GROQ
    if prov == 'ollama' and (not modelo or ':' not in str(modelo)):
        modelo = modelo if (modelo and ':' in str(modelo)) else MODELO_OLLAMA
    if prov == 'groq':
        return _chat_groq(system, prompt, modelo, json_mode, temperatura)
    return _chat_ollama(system, prompt, modelo, json_mode, temperatura)
